...
if [ ! -d "/src" ]; then
  echo  "代码行数: 前端: $(cloc client | sed -n -e "/^SUM/p"|awk -F' ' '{print $NF}') \t node: $(cloc --exclude-dir=thrift server | sed -n -e "/^SUM/p"|awk -F' ' '{print $NF}') \n"
else
  echo  "代码行数: 前端: $(cloc src | sed -n -e "/^SUM/p"|awk -F' ' '{print $NF}') \n\t node: 0"
fi

echo "代码维护者列表:\n $(git log --since="2021-06-23" | grep "^Author: " | awk '{print $2}' | sort | uniq -c | sort -k1,1nr)"
